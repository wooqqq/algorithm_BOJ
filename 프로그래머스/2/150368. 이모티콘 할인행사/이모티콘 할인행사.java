...
class Solution {
    static int[] percent = {0, 10, 20, 30, 40}; // 할인율
    static int totalJoin = 0, totalPrice = 0, min = Integer.MAX_VALUE;
    ...
    public int[] solution(int[][] users, int[] emoticons) {
        ...
        int[] answer = {totalJoin, totalPrice}; // 가입 유저 수, 매출액
    ...
    private void cal(int[][] users, int[] emoticons, int[] discounts) {
        ...
        if (join > totalJoin) {
            totalJoin = join;
            totalPrice = price;
        } else if (join == totalJoin && price > totalPrice) {
            totalPrice = price;
        }
    }
